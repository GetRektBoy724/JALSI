...
public class DInvokeCore {
    ...
    public static NTSTATUS NtProtectVirtualMemory(IntPtr ProcessHandle, ref IntPtr BaseAddress, ref IntPtr RegionSize, UInt32 NewProtect, ref UInt32 OldProtect) {
        // Craft an array for the arguments
        OldProtect = 0;
...
public class JALSI {
	...
	public static bool LocalInject(byte[] syelkot) {
	    ...
	    if (updateMemSettingForAllocatedMem == DInvokeCore.NTSTATUS.Success) {
	    	...
	    	Console.WriteLine("[-] Failed to complete process for allocating memory with error of {0}! [-]", updateMemSettingForAllocatedMem);
	    	return false;
	    }
	}
}